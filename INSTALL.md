...
`next` is the npm prerelease channel and currently resolves to `corgispec@3.0.0-rc.5`. Use `npm install -g corgispec@3.0.0-rc.5` when an exact, reproducible version is required. Do not use the unqualified package name for this upgrade: `corgispec` and `corgispec@latest` remain on stable `2.4.3`.
