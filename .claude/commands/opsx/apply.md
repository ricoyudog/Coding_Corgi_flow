---
name: "OPSX: Apply"
description: Implement tasks from an OpenSpec change, one Task Group at a time
category: Workflow
...
**Input**: Optionally specify a change name (e.g., `/opsx:apply add-auth`). If omitted, infer from context.
...
3. **Dispatch to platform skill**
   ...
   - If `schema: gitlab-tracked` → Follow the instructions in the **openspec-apply-change** skill
   - If `schema: github-tracked` → Follow the instructions in the **openspec-gh-apply** skill
