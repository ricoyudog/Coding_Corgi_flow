...
| 優先級 | 目標 | 定義 |
|---|---|---|
| P0 | **向後相容** | 重構期間 `/opsx-*` 命令和 `install-skills.sh` 持續正常運作 |
...
|---|---|---|---|
| `openspec-install` | universal | Atom | 驗證 prerequisites，複製 managed fileset |
| `openspec-explore` | gitlab | Atom | Read-only context mode |
| `openspec-gh-explore` | github | Atom | Read-only context mode |
| `openspec-propose` | gitlab | Molecule | 產生 artifacts + 建立 GitLab issues |
| `openspec-gh-propose` | github | Molecule | 產生 artifacts + 建立 GitHub issues |
| `openspec-apply-change` | gitlab | Molecule | 執行一個 Task Group checkpoint |
| `openspec-gh-apply` | github | Molecule | 執行一個 Task Group checkpoint |
| `openspec-review` | gitlab | Molecule | Quality checks + human gate |
| `openspec-gh-review` | github | Molecule | Quality checks + human gate |
| `openspec-archive-change` | gitlab | Atom | Close issues + move to archive |
| `openspec-gh-archive` | github | Atom | Close issues + move to archive |
...
|---|---|---|
| `artifact-creation.md` | openspec-propose | → `openspec-cli-runner` atom 內化 |
| `gitlab-issues.md` | openspec-propose | → `sync-issue-gl` atom |
| `checkpoint-flow.md` | openspec-apply-change, openspec-gh-apply | → `parse-tasks` + molecule 流程 |
| `delegation-strategy.md` | openspec-apply-change, openspec-gh-apply | → 留在 `opsx-apply` molecule 內 |
| `issue-sync.md` | openspec-apply-change, openspec-gh-apply | → `sync-issue-gl`, `sync-issue-gh` atoms |
...
| `quality-checks.md` | openspec-review（注意：gh-review 缺此檔案） | → 留在 `opsx-review` molecule 內 |
| `review-decisions.md` | openspec-review（注意：gh-review 缺此檔案） | → 留在 `opsx-review` molecule 內 |
| `repair-flow.md` | openspec-review（注意：gh-review 缺此檔案） | → 留在 `opsx-review` molecule 內 |

**現有不一致：** `openspec-gh-review` 缺少 `quality-checks.md`、`review-decisions.md`、`repair-flow.md`（可能 inlined 在 SKILL.md 中）。合併為 `opsx-review` molecule 時統一補全。

**鏡像不一致：** `.codex/skills/` 缺少 `openspec-install`（10/11 skills）。Phase 4 清理時一併修正。
...
```
.opencode/skills/
├── openspec-apply-change/          # GitLab apply
│   ├── SKILL.md
│   └── references/
│       ├── checkpoint-flow.md
│       ├── issue-sync.md
│       └── worktree-discovery.md
├── openspec-gh-apply/              # GitHub apply（~90% 重複）
│   ├── SKILL.md
│   └── references/
│       └── issue-sync.md
├── openspec-propose/               # GitLab propose
├── openspec-gh-propose/            # GitHub propose（~90% 重複）
├── openspec-review/                # GitLab review
├── openspec-gh-review/             # GitHub review（~90% 重複）
├── openspec-archive-change/        # GitLab archive
├── openspec-gh-archive/            # GitHub archive（~90% 重複）
├── openspec-explore/               # GitLab explore
├── openspec-gh-explore/            # GitHub explore（~90% 重複）
└── openspec-install/               # universal install
```
...
**從何抽取：** 每個 `/opsx-*` command wrapper 的開頭都做 config 解析。抽取後 command wrapper 變成純 dispatch，config 邏輯集中在 atom。
...
**從何抽取：** `openspec-apply-change/references/worktree-discovery.md`。目前 apply/review/archive/explore 四個 phase 都有各自的 worktree 解析邏輯。

### Atom 3: `openspec-cli-runner`

| 屬性 | 值 |
|---|---|
| **slug** | `openspec-cli-runner` |
| **tier** | `atom` |
| **platform** | `universal` |
...
**從何抽取：** `openspec-gh-propose`、`openspec-gh-apply`、`openspec-gh-review`、`openspec-gh-archive` 都在做 GitHub issue 操作。四個 skills 的 issue 邏輯合併為一個 atom。
...
**從何抽取：** `openspec-propose`、`openspec-apply-change`、`openspec-review`、`openspec-archive-change` 的 GitLab issue 操作。
...
### Molecule 1: `opsx-propose`

```json
{
  "slug": "opsx-propose",
  "tier": "molecule",
  "depends_on": ["resolve-config", "resolve-worktree", "openspec-cli-runner",
...
3. `openspec-cli-runner` → `openspec new change <name>`
4. 循環 `openspec-cli-runner` → `openspec status` + `openspec instructions` 依序產生：
...
9. **輸出：** Planning package complete，提示使用者執行 `/opsx-apply`

**合併來源：** `openspec-propose` + `openspec-gh-propose`

### Molecule 2: `opsx-apply`
...
2. `openspec-cli-runner` → `openspec instructions apply`
...
11. **輸出：** Checkpoint complete，STOP，提示使用者執行 `/opsx-review`

**硬性約束：** 每次只執行一個 Task Group。不允許跨 group 執行。

**合併來源：** `openspec-apply-change` + `openspec-gh-apply`

### Molecule 3: `opsx-review`
...
8. **輸出：**
   - Approve + more groups → 提示 `/opsx-apply`
   - Approve + all done → 提示 `/opsx-archive`
   - Reject → 提示 `/opsx-apply`

**Reference docs 保留在 molecule 內：** `quality-checks.md`、`review-decisions.md`、`repair-flow.md` 是 review-specific 邏輯，不需抽成 atom。

**合併來源：** `openspec-review` + `openspec-gh-review`

### Molecule 4: `opsx-archive`
...
**合併來源：** `openspec-archive-change` + `openspec-gh-archive`

### Molecule 5: `opsx-explore`
...
**合併來源：** `openspec-explore` + `openspec-gh-explore`

### Molecule 6: `opsx-install`
...
**來源：** `openspec-install`（幾乎不需要改動，只是加上 skill.meta.json 和移到 molecules/ 目錄）
...
|---|---|---|
| `opsx-propose.md` | `openspec-propose` 或 `openspec-gh-propose`（依 platform） | `opsx-propose`（unified） |
| `opsx-apply.md` | `openspec-apply-change` 或 `openspec-gh-apply` | `opsx-apply`（unified） |
| `opsx-review.md` | `openspec-review` 或 `openspec-gh-review` | `opsx-review`（unified） |
| `opsx-archive.md` | `openspec-archive-change` 或 `openspec-gh-archive` | `opsx-archive`（unified） |
| `opsx-explore.md` | `openspec-explore` 或 `openspec-gh-explore` | `opsx-explore`（unified） |
| `opsx-install.md` | `openspec-install` | `opsx-install`（unified） |

Phase 3 後，command wrappers 簡化為單一 skill 引用（無 platform 分支）。`.claude/commands/opsx/` 同步更新。
...
**交付物：**
- 6 個新 molecule 目錄（`opsx-propose`、`opsx-apply`、`opsx-review`、`opsx-archive`、`opsx-explore`、`opsx-install`）
- 更新 6 個 `.opencode/commands/opsx-*.md` 和 `.claude/commands/opsx/*.md` 指向新 molecules
- 更新 `ds-skills install` 安裝新結構
- 補全 `.codex/skills/` 缺失的 `openspec-install`
