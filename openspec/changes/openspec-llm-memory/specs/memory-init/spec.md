...
The system SHALL create a complete memory directory structure when `openspec-memory-init` is invoked, producing `memory/` (3 files) and `wiki/` (7 subdirectories with index files) without overwriting any existing files.

#### Scenario: Fresh project initialization
- **WHEN** user runs `openspec-memory-init` on a project with no existing `memory/` or `wiki/` directories
...
- **WHEN** user runs `openspec-memory-init` on a project where `memory/session-bridge.md` already exists
...
- **WHEN** user runs `openspec-memory-init` on a project that has `CLAUDE.md` or `README.md`
...
- **WHEN** `openspec-memory-init` runs and `CLAUDE.md` exists
...
- **WHEN** `openspec-memory-init` runs and `CLAUDE.md` already contains `## Session Memory Protocol`
- **THEN** system does not duplicate the section and reports it already exists

### Requirement: Install integration
The system SHALL invoke `openspec-memory-init` as part of `openspec-install` by default, with an opt-out mechanism.

#### Scenario: Default install includes memory
- **WHEN** user runs `openspec-install` without flags
- **THEN** system completes standard install AND runs `openspec-memory-init`

#### Scenario: Opt-out via flag
- **WHEN** user runs `openspec-install --no-memory`
- **THEN** system completes standard install WITHOUT running `openspec-memory-init`
