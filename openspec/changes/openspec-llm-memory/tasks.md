...
- [x] 1.1 Create `openspec-memory-init` skill directory in `.opencode/skills/` with `SKILL.md` and `skill.meta.json` (tier: atom, platform: universal)
...
- [x] 1.6 Sync skill to `.claude/skills/openspec-memory-init/` and `.codex/skills/openspec-memory-init/` (three-directory obligation)
- [x] 1.7 Add `/opsx-memory-init` slash command in `.opencode/commands/`
...
- [x] 3.1 Modify `openspec-install` SKILL.md to call `openspec-memory-init` after standard install completes
- [x] 3.2 Add `--no-memory` flag handling: skip memory-init when flag is present
- [x] 3.3 Update `openspec-install` skill.meta.json to add `openspec-memory-init` as a dependency
...
- [x] 4.1 Create `openspec-lint` skill directory in `.opencode/skills/` with `SKILL.md` and `skill.meta.json` (tier: molecule, platform: universal)
...
- [x] 4.12 Add `/opsx-lint` slash command in `.opencode/commands/`

## 5. Memory Ask Skill

- [x] 5.1 Create `openspec-ask` skill directory in `.opencode/skills/` with `SKILL.md` and `skill.meta.json` (tier: molecule, platform: universal)
...
- [x] 5.8 Add `/opsx-ask` slash command in `.opencode/commands/`

## 6. Memory Extract Skill & Archive Integration

- [x] 6.1 Create `openspec-memory-extract` skill directory in `.opencode/skills/` with `SKILL.md` and `skill.meta.json` (tier: atom, platform: universal)
...
- [x] 6.7 Modify `openspec-archive` SKILL.md to invoke `openspec-memory-extract` before closing
- [x] 6.8 Update `openspec-archive` skill.meta.json dependencies
