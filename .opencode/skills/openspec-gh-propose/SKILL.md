---
name: openspec-gh-propose
description: Propose a new change with all artifacts generated in one step and tracked via GitHub issues. Use when the user wants to quickly describe what they want to build and get a complete proposal with design, specs, and tasks ready for implementation.
license: MIT
...
After explicit review of the proposal/spec/design/tasks package, continue with /opsx-apply.
...
6. **Create artifacts in sequence until all implementation-required artifacts are complete**
...
8. **Closeout: Create GitHub issues (if gh available and tracking not already configured)**
   ...
   c. If gh is available and tracking is not yet configured:
      ...
      - Build one parent issue body with:
        ...
        - `**Task Groups**` table with a Status column:
          ...
          |-------|------|-------|--------|
          | 1 | Setup | #<child_1_number> | backlog |
          | 2 | Core Implementation | #<child_2_number> | backlog |
          ```
          All groups start with Status `backlog`. Status values: `backlog` → `in-progress` → `review` → `done`.
        - `**Progress:**` 0/N groups completed
        - `**Child Issues**` — task list: `- [ ] #<number> Group N: name`
        - `**Conclusion**`
      ...
      - Post an initial note on the parent issue:
        ```bash
        gh issue comment <parent_issue_number> --body "Planning complete. This change has one parent issue and one child issue per Task Group. Review the proposal/spec/design/tasks package. After explicit approval, run /opsx-apply to begin implementation."
        ```
      - Update `proposal.md` in the `## GitHub Issue` section with the parent issue link

9. **Closeout: Save worktree metadata (if isolation mode is worktree)**
...
- Prompt: "Review the proposal/spec/design/tasks package. After explicit approval, run `/opsx-apply` to start implementation."
...
- If `.github.yaml` already exists, do not create duplicate issues
