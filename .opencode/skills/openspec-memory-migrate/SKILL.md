...
license: MIT
compatibility: Requires memory/ and wiki/ directories (run openspec-memory-init first).
metadata:
  author: openspec
  version: "1.0"
  generatedBy: "1.0.0"
...
- After `openspec-memory-init` on an existing project with accumulated knowledge
- When a project has `docs/`, archived changes, or existing .md files that should feed the memory system
- Via `/opsx-migrate` slash command

Do not use this skill to initialize empty memory (use `openspec-memory-init`), run lint checks, or extract from active/in-progress changes.
...
- If not, create a pattern file following the format in `openspec-memory-extract` skill
...
2. Run `/opsx-lint` to validate memory health
3. Consider running `/opsx-propose` for your next change — memory will now provide context
```
