...
- **Automated verify gate：** 自動執行 lint、build、tests 與 spec coverage，未通過則阻擋 review
- **5-axis review：** Architecture、Security、Performance、Code Quality、Completeness 五軸品質檢查，以證據驅動，提供 approve/reject/discuss 決策
...
這張圖只呈現 commands 之間的 handoff points。實際上，`/corgi-propose` 會在 planning artifacts 完成後 close out 成可追蹤的 handoff state，`/corgi-apply` 會把單一 Task Group 做到 implementation 與 closeout 完成後再停止，`/corgi-verify` 會執行自動化驗證（lint、build、tests、spec coverage），若未通過則阻擋 review，而 `/corgi-review` 則會先蒐集證據，再要求明確決策，最後套用使用者同意的狀態轉換。
...
| OpenCode `/corgi-verify` / Claude `/corgi:verify` | 自動化品質關卡：lint、build、tests、spec coverage，未通過則阻擋 review |
| OpenCode `/corgi-review` / Claude `/corgi:review` | 五軸 review（Architecture、Security、Performance、Code Quality、Completeness）、明確決策、套用狀態轉換 |
...
| Review | 無 | 五軸自動化品質檢查（Architecture、Security、Performance、Code Quality、Completeness），加上 verify gate、approve/reject/discuss 與 repair loop |
