...
6. **Create GitHub issues (if gh available and tracking not already configured)**
   ...
   c. If gh is available and tracking is not yet configured:
      ...
      - Update the parent issue description so its Task Groups table and Child Issues task list includes the created child issue numbers
        ...
        ```
      - Save `.github.yaml` in the change directory:
        ```yaml
        parent:
        ...
        groups:
          - number: 1
            name: "Setup"
            issue_number: <child_1_number>
            url: <child_1_url>
          - number: 2
            name: "Core"
            issue_number: <child_2_number>
            url: <child_2_url>
        ```
      - Post an initial note on the parent issue:
        ```bash
        gh issue comment <parent_number> --body "Planning complete. This change has one parent issue and one child issue per Task Group. Run /opsx-apply to begin implementation."
        ```
...
6.5 **Save worktree metadata (if isolation mode is worktree)**
   ...
   ```bash
   gh issue edit <parent_number> --body "$UPDATED_PARENT_WITH_WORKTREE"
   ```
